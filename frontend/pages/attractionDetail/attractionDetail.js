...
Page({
  ...
  onLoad(options) {
    ...
    query.select('.attraction-name').boundingClientRect((rect) => {
      const parentWidth = rect.width; // 获取父容器宽度
      const maxWidth = parentWidth * 0.8; // 假设标题最大宽度是父容器的80%

      query.select('.attraction-title').boundingClientRect((res) => {
        const scaleFactor = Math.min(1, maxWidth / res.width); // 比较最大宽度和实际宽度的比例
        if (this.data.scaleFactor == 1) {
          this.setData({
